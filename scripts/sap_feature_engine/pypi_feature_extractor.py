...
#        self.stopwords = set(nltk.corpus.stopwords.# dangerous token (FIXED PATH)
#        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
#        danger_path = os.path.join(BASE_DIR, "resources", "dangerous_tokens.json")

#        with open(danger_path, "r") as file:
#             self.dangerous_token = json.load(file)words('english'))
...
#            self.dangerous_token = json.load(file)
    ...
    def merge_py_of_same_package(self, database: pd.DataFrame) -> pd.DataFrame:
        if 'code' not in database.columns:
            database['code'] = ""

        p_database = (
            database
            .groupby('Package Name')['code']
            .apply(lambda x: '\n'.join(x.astype(str)))
            .reset_index()
        )

        p_database['Number of words'] = p_database['code'].apply(
