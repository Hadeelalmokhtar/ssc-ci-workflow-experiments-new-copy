...
from scripts.sap_feature_engine import utilities_functions
import pandas as pd
import numpy as np
import statistics
import stat
...
class NPM_Feature_Extractor:
    ...
    def __init__(self) :
        ...
        danger_path = os.path.join(BASE_DIR, "resources", "dangerous_tokens.json")

        with open(danger_path, "r") as file:
...
#            self.dangerous_token = json.load(file)
            ...
            for file in files:
                if file.endswith(".tgz"):
                    if os.path.getsize(os.path.join(self.path_to_scan,file)) > 0:
                        ...
                        output_dir="".join((self.path_to_scan,"/",file.split(".tgz")[0]))
    ...
    def merge_js_of_same_package(self, database: pd.DataFrame) -> pd.DataFrame:
        # function that merge .js file inside the same package 
        ...
        #url = database.groupby(['Package Name'], as_index=False)['url'].sum()
    ...
    def extract_feature_from_package_json(self) -> pd.DataFrame:
        ...
        for file_path in files_path:
            ...
            if file_path.split('/')[-1] == "package.json":
                ...
                id = [s.replace("'", '').replace('"', '') for s in id]
                id_ = ' '.join(id)
                ...
                for value in strs:
                    count_base64 += len(utilities_functions.contains_base64(value))
                    count_IP += len(utilities_functions.contains_IPAddress(value))
                    byte += len(utilities_functions.contains_dangerous_token(value, self.dangerous_token))
    ...
    def p_db_benign_md(self,database):
        p_database= database.groupby(['Package Name'], as_index=False)['code'].agg('\n'.join)
        p_database['Number of words'] = p_database["code"].apply(lambda n: len(n.split()))
        l_database = database.groupby(['Package Name'], as_index=False)['lines'].sum()
        base = database.groupby(['Package Name'], as_index=False)['base64'].sum()
    ...
    # function for shannon entropy extraction, url heterogeneous strings and identifiers 
    def extraction(self,database,alphabetic_string,base_string,alphabetic_id,base_id):
        extractor = URLExtract()
        # repository for NPM 
        database['repository'] = pd.Series([1 for x in range(len(database.index))])
        ...
        for i in range(len(database)): 
                ...
                for k in range(0,len(string_md)):
                        url_md+=len(extractor.find_urls(string_md[k]))
